"""
TODO: Check i CSVLogger works as it should
"""
"""
...
Modified by: "Christine Kiran Kaushal"
date: 25th Feb 19
e-mail: christine.kiran@gmail.com
"""
...
class BaseLogger:
    ...
    def _log(self, summary, it_num, log_type, *args, **kwargs):
...
class CSVLogger(BaseLogger):
    ...
    def _log(self, summary, it_num, log_type):
        ...
        #self.logs = self.logs.fillna(0)
        #print('SUMMARY ER TYPE: ',type(summary))
        for name, s in summary.items():
            self.logs[log_type][name] = np.mean(s)
            #try:
            #    self.logs[log_type][name] = self.logs[log_type][name].append(pd.Series([np.mean(s)]), ignore_index=True).values
            #except AttributeError:
            #    self.logs[log_type][name] = pd.Series([np.mean(s)])
